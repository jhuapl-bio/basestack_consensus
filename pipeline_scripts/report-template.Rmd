...
problematic_sites <- read.vcfR(paste0(resource_path,"/problematic_sites_sarsCov2.vcf"), verbose = FALSE, skip=24)
...
amplicons <- read.table(paste0(run_path,"/artic-pipeline/run_stats/amplicons.txt"), sep="\t", header=TRUE)
...
if(!is.null(mutation_no_low_coverage)) {
...
# mutation_collapse[select_many] <- paste(mutation_counts[select_many], "SNPs")
  ...
  mutation_collapse <- NULL
  mutation_counts <- NULL
  select_many <- NULL
}
if(!is.null(postfilt)) {
...
plot_amplicon_hist <- function() {
  ...
  fill <- "black"
  barplot(h$counts, col=fill, border=fill, log='y', axes=F,
          xlab="average read depth across amplicon (average number of reads)",
          main="",
          ylab="", cex.lab=1, ylim=c(0.5,1e3), xlim=c(0,300*slope))
...
print_igv_plot <- function(row) {
  ...
  if(nchar(as.character(row$Variants)) > 0) {
    variants <- strsplit(as.character(row$Variants), ", ")[[1]]
  } else {
    variants <- NULL
  }
  
  if(length(flags) + length(variants) > 0) {
    for(flagString in flags) {
      nt <- strsplit(flagString, "=")[[1]][1]
      title <- paste0("\n\n### ", colorlevels(flag), "{[", flag, "]: ", sample, " ", flagString)
      SNP <- variants[unlist(sapply(variants, function(s){return(gsub('[a-zA-Z]', '', s))})) %in% nt]
      if(length(SNP) > 0) {
        title <- paste0(title, " - ", SNP)
      }
      alleles <- postfilt_all$ont_alleles[postfilt_all$pos == nt & postfilt_all$sample == sample]
      if(length(alleles) > 0) {
        title <- paste0(title, " - ", alleles)
      }
      cat(paste0(title, "}\n\n"))
      if(nt != "see full output") {
        nt <- as.numeric(nt)-1
        pngFn <- paste0(run_path,"/artic-pipeline/run_stats/igv/",sample_barcode,"/",
                        genome_ref$reference$accession,".3_",nt,".png")
    ...
    if(variants[1] != "see full output") {
      for(variant in variants) {
        REF <- substr(variant, 1, 1)
        ALT <- substr(variant, nchar(variant), nchar(variant))
        nt <- gsub('[a-zA-Z]', '', variant)
        alleles <- postfilt_all$ont_alleles[postfilt_all$pos == nt & postfilt_all$sample == sample]
        if(!(nt %in% sapply(flags, function(s){return(strsplit(s, "=")[[1]][1])}))){
          flag <- "Yes"
          title <- paste0("\n### ", colorlevels(flag), "{[", flag, "]: ", sample, " - ", variant, " - ", alleles, "}\n")
          cat(title)
          nt <- as.numeric(nt)-1
          pngFn <- paste0(run_path,"/artic-pipeline/run_stats/igv/",sample_barcode,"/",genome_ref$reference$accession,".3_",nt,".png")
          if(file.exists(pngFn)) {
            cat(paste0("\\includegraphics[width=13cm]{",pngFn,"}\n"))
          } else {
            cat(paste0("\nWarning: file does not exist: ",paste(tail(strsplit(pngFn,"/")[[1]],5),collapse="/"),"\n"))
...
## Demultiplex summary
```{r, barcode-barplot, echo=FALSE, message=FALSE, warning=FALSE, fig.height=3, fig.width=6, background="white"}
...
Amplicons from NTCs are used to determine an average depth threshold for amplicons from experimental samples.
...
## Flag matrix
```{r, flag-matrix, results="hide", echo=FALSE, warning=FALSE, external=FALSE, fig.height=8, dev='cairo_pdf'}
tryCatch({
  plot_flag_matrix()
}, warning = function(w){
}, error = function(e) {
...
## Length filtering
```{r, length-protocol, echo=FALSE, results='asis'}
gather_config_fn <- paste0(run_path,"/artic-pipeline/2-length-filter/gather-config.yml")
gather_config <- read.table(gather_config_fn, sep=" ")

amplicon_min <- gather_config[gather_config[,1] == "ampliconMin:",2]
amplicon_max <- gather_config[gather_config[,1] == "ampliconMax:",2]
filter_min <- gather_config[gather_config[,1] == "filterMin:",2]
filter_max <- gather_config[gather_config[,1] == "filterMax:",2]

cat(paste0("Shortest amplicon: ", amplicon_min, " nt\n\n"))
cat(paste0("Longest amplicon: ", amplicon_max, " nt\n\n"))

cat("```\n")
cat("artic guppyplex \\\n")
cat("  --skip-quality-check \\\n")
cat(paste0("  --min-length ",filter_min," \\\n"))
cat(paste0("  --max-length ",filter_max," \\\n"))
cat("  --directory \"$demux_dir\"/\"$barcode\" \\\n")
cat("  --prefix \"$gather_dir\"/\"$name\"\n")
cat("```\n")
```
