...
annotation_sample$lineage[!grepl("NTC", sample_barcodes, fixed=T)] <- as.matrix(clade_report$lineage)
...
annotation_snp$problematic <- rep("", length(combined_rows))
annotation_snp$problematic[combined_rows %in% problematic_sites@fix[,2]] <- problematic_sites@fix[problematic_sites@fix[,2] %in% combined_rows,7]
annotation_snp$problematic <- factor(annotation_snp$problematic)
...
#  text_matrix[row_sel, col_sel] <- flag_matrix[flag_row_order[row_order[row_sel]], flag_col_order[col_order[col_sel]]]
  ...
  g <- grid.get(grid.ls(grid.force())$name[3])
...
\textcolor{myRed}{No = Red} = Incomplete genome (<25k nucleotides)
