...
depth_norm <- read.table(paste0(run_path,"/artic-pipeline/run_stats/depth-norm-all.txt"), sep="\t", fill=TRUE)
...
snpeff_report <- read.table(paste0(run_path, "/artic-pipeline/run_stats/final_snpEff_report.txt"), sep="\t", header=TRUE, comment.char="")
...
threshold <- postfilt_all$depth_thresh[1]
below_threshold <- function(v) { return(v<threshold) }
amplicon_region <- depth_norm[,3] >= 55 & depth_norm[,3]<=29836
low_coverage_regions <- tapply(depth_norm[amplicon_region,4], depth_norm[amplicon_region,1], below_threshold)
...
for(i in seq_along(low_coverage_regions)) {
  low_coverage_rows <- dimnames(mutation_no_low_coverage)[[1]] %in% (which(low_coverage_regions[[i]])+54)
  low_coverage_cols <- dimnames(mutation_no_low_coverage)[[2]] %in% sample_barcodes[i]
...
mutation_collapse <- apply(mutation_no_low_coverage, 2, f)
...
mutation_table <- mutation_no_low_coverage[,!select_many]
mutation_table <- mutation_table[rowSums(mutation_table != "") != 0,]
mutation_bool <- mutation_no_low_coverage_bool[,!select_many]
mutation_bool <- mutation_bool[rowSums(mutation_bool > 0) != 0,]
if(sum(select_many)>0) {
  mutation_table_many <- matrix(mutation_no_low_coverage[,select_many], ncol=sum(select_many))
  dimnames(mutation_table_many)[[1]] <- rownames(mutation_no_low_coverage)
  ...
  mutation_bool_many <- matrix(mutation_no_low_coverage_bool[,select_many], ncol=sum(select_many))
  dimnames(mutation_bool_many)[[1]] <- rownames(mutation_no_low_coverage_bool)
...
mut_rows <- dimnames(mutation_bool)[[1]]
mut_cols <- gsub("(_NB[0-9]{2})?(\\*)?", "", dimnames(mutation_bool)[[2]])

combined_rows <- sort(as.numeric(c(postfilt_rows, mut_rows[!(mut_rows %in% postfilt_rows)])))
combined_cols <- postfilt_cols

annotation_sample <- data.frame(Flag = flagList[-13])
...
snpeff_annotations[combined_rows %in% snpeff_report$POS] <- as.character(snpeff_report$ANN[snpeff_report$POS %in% combined_rows])
annotation_snp <- data.frame(common.SNP = combined_rows %in% important_positions$POS, SnpEff = snpeff_annotations)
rownames(annotation_snp) <- combined_rows
annotation_snp$common.SNP <- factor(annotation_snp$common.SNP)
levels(annotation_snp$common.SNP) = c("other", "common")
annotation_snp$SnpEff <- factor(snpeff_annotations, levels = c(levels(snpeff_report$ANN), "none"))

# create color object for annotations
annotation_colors <- list(Flag = flagColors[flagColors != flagColors["NA"]], common.SNP = c("common"="#000000", "other"="#FFFFFF"), Variants = c("#FFFFFF", "gray30", "gray70"), SnpEff = c(brewer.pal(n=length(levels(snpeff_report$ANN)), name="Dark2"), "white"))
names(annotation_colors$SnpEff) <- levels(annotation_snp$SnpEff)
#-------------------------------------------------
```
...
print_low_depth_regions <- function() {
  ...
  get_regions <- function(i){
    d <- depth_norm[amplicon_region,]
    l <- levels(factor(d[,1]))
    r <- d[d[,1]==l[i] & low_coverage_regions[[i]], 3]
    if(length(r) > 0) {
  ...
  cat(paste0("**Depth threshold**: ", threshold, "\n"))
...
plot_homoplasy_heatmap <- function() {
  ...
  sample_sel <- summary$Sample %in% gsub("(_NB[0-9]{2})?(\\*)?", "", dimnames(plot_matrix)[[2]])
 ...
 #         legend_labels=c("Wuhan-Hu-1","Variant","Ambiguous"),
  ...
  grid.text("Variant position (nucleotide)", x=0.73, y=0.35, just="center", rot=-90, gp=gpar(col="black", fontsize=12))
}
#-------------------------------------------------
...
plot_flag_matrix <- function() {
  ...
  plot_matrix[o, o.col][combined_rows %in% mut_rows, combined_cols %in% mut_cols] <- mutation_bool[mut_rows %in% combined_rows, mut_cols %in% combined_cols]
...
#  ntc_sel <- nchar(as.matrix(ntc_matrix))[,-1]>0
  ...
  vc_sel_ms <- as.matrix(vc_matrix)[,-1] == 'mismatch(m+s)'
  vc_sel_mn <- as.matrix(vc_matrix)[,-1] == 'mismatch(m+n)'
  vc_sel_ns <- as.matrix(vc_matrix)[,-1] == 'mismatch(n+s)'
  ...
  flag_matrix[vc_sel_ms] <- paste0(flag_matrix[vc_sel_ms], "ms")
  flag_matrix[vc_sel_ns] <- paste0(flag_matrix[vc_sel_ns], "ns")
  flag_matrix[vc_sel_mn] <- paste0(flag_matrix[vc_sel_n], "mn")
...
#  text_matrix[row_sel, col_sel] <- flag_matrix[flag_row_order[row_order[row_sel]], flag_col_order[col_order[col_sel]]]
  text_matrix[row_sel, col_sel] <- flag_matrix[flag_row_order, flag_col_order]

  x <- rep(seq(0.095, 0.58, length.out=dim(text_matrix)[2]), each=dim(text_matrix)[1])
  y <- rep(seq(0.961, 0.131, length.out=dim(text_matrix)[1]), dim(text_matrix)[2])
  grid.text(text_matrix, x=x, y=y, gp=gpar(col="black", fontsize=7))

#  grid.text(paste(shapes$triangles[1:5], collapse=""), x=0.095, y=0.961, gp=gpar(col="black", fontsize=7))
#  grid.text(paste(shapes$circles[3:7], collapse=""), x=0.58, y=0.131, gp=gpar(col="black", fontsize=7))
  grid.text("Variant position (nucleotide)", x=0.7, y=0.35, just="center", rot=-90, gp=gpar(col="black", fontsize=12))
}
#-------------------------------------------------
...
### Samples containing positions with depth less than 2x NTC median depth
\label{low_depth}
...
### Nextstrain tanglegram
![](`r "/home/tom/data/ref/ncov/sequencing_runs/1-GLOBAL_MDHP_tanglegram.png"`)

### Nextstrain global transmission
![](`r "/home/tom/data/ref/ncov/sequencing_runs/0-GLOBAL_Transmission_map.png"`)

### MDHP-only time tree
![](`r "/home/tom/data/ref/ncov/sequencing_runs/2-MDHP_AA_MUTS_tree.png"`)

### MDHP-only divergence tree
![](`r "/home/tom/data/ref/ncov/sequencing_runs/3-MDHP_AA_MUTS_divergence.png"`)
...
**Flags**: MAF (Minor allele frequency > 0.15), new (present in < 2 GISAID sequences), mismatch (mismatches between variant callers where variant is present in nanopolish (n), medaka (m), and/or samtools(s))
...
**Variants**: \textcolor{variantBlue}{$\blacksquare$} = Variant in consensus, \textcolor{variantRed}{$\blacksquare$} =  Ambiguity in consensus (at depth above threshold outlined in section \ref{low_depth} on page \pageref{low_depth}) ; **Flags**: $\blacksquare$ = MAF (Minor allele frequency > 0.15), $\bigcirc$ = new (present in < 2 GISAID sequences), s|m|n (mismatches between variant callers where variant is present in samtools (s), medaka (m), nanopolish (n))
