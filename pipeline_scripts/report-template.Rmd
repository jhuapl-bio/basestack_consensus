...
summary$Depth[!uncalled_row] <- c(paste0(format(round(tapply(depth_all[,4],factor(depth_all[,1], levels=sample_barcodes), sum)/29903, 0), big.mark=",", trim=T), "x"))
...
if(dim(clade_report)[1]>0) {
  summary$Lineage[clade_report$taxon %in% sample_barcodes] <- as.matrix(clade_report$lineage)
}
...
mutation_table <- subset(mutation_no_low_coverage, subset=rowSums(mutation_no_low_coverage != "") != 0, select=!select_many)
mutation_bool <- subset(mutation_no_low_coverage_bool, subset=rowSums(mutation_no_low_coverage_bool > 0) != 0, select=!select_many)
if(sum(select_many)>0) {
  mutation_table_many <- subset(mutation_no_low_coverage, select=select_many)
  new_cols <- subset(mutation_no_low_coverage, subset=rowSums(mutation_no_low_coverage > 0) != 0, select=select_many)
  if(dim(new_cols)[2] > 0) {
    if(dim(new_cols)[2] == 1) {
  ...
  if(dim(new_cols)[2] > 0) {
    if(dim(new_cols)[2] == 1) {
      new_names <- list(sapply(dimnames(mutation_bool_many)[[2]], paste0, "*"))
    } else {
      new_names <- sapply(dimnames(mutation_bool_many)[[2]], paste0, "*")
    }
    dimnames(new_cols)[[2]] <- new_names
...
for(i in sample_barcodes) {
	...
	for(a in 1:dim(amplicons)[[1]]) {
		sel <- as.numeric(d[,2]) >= amplicons$unique.start[a] & as.numeric(d[,2]) <= amplicons$unique.stop[a]
		if(sum(sel) > 0) {
...
plot_barcode_barplot <- function() {
  ...
  ggplot(df, aes(x = reorder(sample, -sample), y = log10(reads), fill = Flag)) +
...
depth_heatmap <- function(plot_sel, title_additional, norm=F) {
  ...
  pheatmap(as.data.frame(plot_dat),
           ...
           annotation_colors=list(Flag = flagColors)
        )
}
#-------------------------------------------------
