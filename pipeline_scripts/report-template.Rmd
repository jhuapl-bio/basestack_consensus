...
problematic_sites <- read.table(paste0(run_path,"/../problematic_sites_sarsCov2.txt"), sep="\t", header=TRUE)
genome_ref <- fromJSON(file=paste0(protocol_path, "/genome.json"))
important_positions <- read.table(paste0(nextstrain_path,"/latest/alpha/clade_related_snps.txt"), sep="\t", header=TRUE)
...
summary$Consensus[summary$Sample %in% postfilt$Sample] <- as.character(postfilt$Status)
