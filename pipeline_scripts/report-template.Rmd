...
plot_flag_matrix <- function() {
  ...
  plot_matrix[o, o.col][combined_rows %in% mut_rows, combined_cols %in% mut_cols] <- mutation_bool_many[mut_rows %in% combined_rows, mut_cols %in% combined_cols]
  ...
  for(i in 1:length(names(flags))) {
    ...
    if(flag == "vc") {
      ...
      symbol <- gsub("mismatch\\((.*)\\)", "\\1", vc_calls)
    } else {
...
**Variants**: \textcolor{variantBlue}{$\blacksquare$} = Variant in consensus, \textcolor{variantRed}{$\blacksquare$} =  Ambiguity in consensus (above depth threshold outlined in section \ref{low_depth} on page \pageref{low_depth}) ; **Flags**: D = depth, $\blacksquare$ = NTC, (m|n|s) = variant present only in (m)edaka, (n)anopolish, (s)amtools, ~ = mixed, % = MAF, B = strand bias, ! = variant in key position, $\circ$ = present in <2 GISAID sequences
