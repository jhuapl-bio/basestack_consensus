...
if(sum(select_many)>0) {
  ...
  if(dim(new_cols)[2] > 0) {
    if(dim(new_cols)[2] == 1) {
      new_names <- list(sapply(dimnames(mutation_table_many)[[2]], paste0, "*"))
    } else {
      new_names <- sapply(dimnames(mutation_table_many)[[2]], paste0, "*")
    }
    dimnames(new_cols)[[2]] <- new_names
...
depth_heatmap <- function(plot_sel, title_additional, norm=F) {
  ...
  sample_sel <- summary$Sample %in% gsub("(_NB[0-9]{2})?(\\*)?", "", sample_barcodes)
  ...
  pheatmap(as.data.frame(plot_dat),
           ...
           labels_col="",
           annotation_row=subset(annotation_sample, sample_sel),
