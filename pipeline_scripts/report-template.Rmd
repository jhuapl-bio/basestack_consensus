...
common_positions <- read.table(paste0(nextstrain_path,"/latest/alpha/alignments.vcf"), sep="\t")
...
#                      fill = rep(c('#2ca25f', '#99d8c9'), 10)[seq(1, length(genome_ref$genes))],
  ...
  for(i in 1:dim(mutation_subset)[2]) {
    ...
    rename_sel <- snpeff_subset$ANN[snpeff_labels_sel] %in% c("upstream_gene_variant","downstream_gene_variant")
