...
if(file.exists(snpeff_reportFn)) {
  ...
  snpeff_report <- data.frame("X.CHROM" = rep("MN908947.3", 2), "POS" = empty_column, "REF" = empty_column, "ALT" = empty_column, "GENE" = empty_column, "ANN" = empty_column, "AA_MUT" = empty_column)
}
...
depth_heatmap <- function(plot_sel, title_additional, norm=F) {
  ...
  # annotation_row <- data.frame(Flag = flagList[!uncalled_row])
...
#-------------------------------------------------
depth_plot_old <- function(plot_sel, title_additional) {
  a <- amplicons
  dat <- depth_all
...
depth_plot <- function(plot_sel, title_additional) {
	...
	for(i in seq(1,dim(depth_plot_all)[1])) {
		...
		abline(h=c(1,10,100,1000,1e4,1e5), col="gray80")
		b <- barplot(bar_data, log='y', axes=F, add=TRUE, col=flagFillDict[barcode], border=NA, ylim=c(1,1e5))
		axis(1, at=b, labels=rep("", length(b)))
		axis(2, at=c(1,10,100,1000,10000,100000), labels=c(1,10,100,"1k","10k","100k"), las=2, cex.axis=1)
		title(ylab="reads", line=2.5, cex.lab=1)
		title(xlab="amplicon", line=1, cex.lab=1)
...
plot_homoplasy_heatmap <- function() {
 ...
 #         legend_labels=c("Wuhan-Hu-1","Variant","Ambiguous"),
  ...
  grid.text("Variant position (nucleotide)", x=0.83, y=0.35, just="center", rot=-90, gp=gpar(col="black", fontsize=12))
}
#-------------------------------------------------
...
#  grid.text(paste(shapes$circles[3:7], collapse=""), x=0.58, y=0.131, gp=gpar(col="black", fontsize=7))
  grid.text("Variant position (nucleotide)", x=0.85, y=0.35, just="center", rot=-90, gp=gpar(col="black", fontsize=12))
}
#-------------------------------------------------
