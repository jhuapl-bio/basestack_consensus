...
depth_trimFn <- paste0(run_path,"/artic-pipeline/run_stats/depth-del-all.txt")
...
for(i in sample_barcodes) {
	...
	for(a in 1:dim(amplicons)[[1]]) {
		sel <- as.numeric(d[,2]) >= amplicons$unique_start[a] & as.numeric(d[,2]) <= amplicons$unique_stop[a]
		if(sum(sel) > 0) {
		  b[a] <- min(as.numeric(d[sel,3]))
		} else {
		  b[a] <- 0
		}
	}
  depth_plot_min <- rbind(depth_plot_min, b)
}
rownames(depth_plot_min) <- sample_barcodes
...
for(sample_barcode in dimnames(depth_plot_min)[[1]]) {
  for(a in which(depth_plot_min[1,] < threshold)) {
    low_coverage_regions[sample_barcode][[1]][seq(amplicons$unique_start[a], amplicons$unique_stop[a], 1)] <- TRUE
  }
  low_coverage_regions[sample_barcode][[1]][seq(1, amplicons$primer_f_stop[1], 1)] <- TRUE
  low_coverage_regions[sample_barcode][[1]][seq(amplicons$unique_start[98], 29903, 1)] <- TRUE
}
...
print_low_depth_regions <- function() {
  ...
  get_regions <- function(i){
    d <- depth_norm
    l <- levels(factor(d[,1]))
    r <- d[d[,1]==l[i] & low_coverage_regions[[i]] & d[,3] >= 55 & d[,3]<=amplicons$primer_r_stop[97], 3]
    if(length(r) > 0) {
...
**Variants**: \textcolor{variantBlue}{$\blacksquare$} = Variant in consensus, \textcolor{variantRed}{$\blacksquare$} =  Ambiguity in consensus (above depth threshold outlined in section \ref{low_depth} on page \pageref{low_depth}) ; **Flags**: D = depth, $\blacksquare$ = NTC, (m|n|s) = present only in (m)edaka, (n)anopolish, (s)amtools, ~ = mixed, % = MAF, B = strand bias, ! = key position, $\circ$ = present in <2 GISAID sequences
