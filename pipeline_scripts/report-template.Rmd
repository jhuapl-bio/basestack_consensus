...
genome_ref <- fromJSON(file=paste0(params$protocol_path, "/genome.json"))
...
amplicon_region <- depth_norm[,3] >= 1 & depth_norm[,3]<=29903
...
mut_cols <- dimnames(mutation_bool)[[2]]
...
snpeff_colors <- c("downstream_gene_variant" = cols[1], "missense_variant" = cols[2], "synonymous_variant" = cols[3], "upstream_gene_variant" = cols[5], "stop_gained" = cols[4], "start_lost" = cols[6], "none" = "white")
...
print_low_depth_regions <- function() {
  ...
  get_regions <- function(i){
    ...
    l <- levels(factor(d[,1]))
    r <- d[d[,1]==l[i] & low_coverage_regions[[i]] & d[,3] >= 55 & d[,3]<=29836, 3]
    if(length(r) > 0) {
...
#-------------------------------------------------
genome_cartoon_overview <- function(mutation_subset=mutation_no_low_coverage) {
  features <- GRanges("MN908947.3", IRanges(sapply(genome_ref$genes, function(g){return(g$start)}), 
                                    width=sapply(genome_ref$genes, function(g){return(g$end-g$start+1)}),
                                    names=names(genome_ref$genes)),
#                      fill = rep(c('#2ca25f', '#99d8c9'), 10)[seq(1, length(genome_ref$genes))],
                      fill = brewer.pal(n=11, name="Set3"),
                      color = "white",
                      height = unit(1/8, "inches"))

  # remove positions covered by multi-site variants
  mutation_subset <- mutation_subset[!(dimnames(mutation_subset)[[1]] %in% c(snpeff_report$POS[sapply(as.character(snpeff_report$REF), nchar)>1]+1)),]
  mutation_subset <- mutation_subset[,-c(7,8,11)]
  mutation_subset <- mutation_subset[apply(nchar(mutation_subset), 1, sum) > 0,]
  ...
  labels[SNP %in% snpeff_subset$POS] <- paste(as.character(snpeff_subset$GENE), as.character(snpeff_subset$AA_MUT), sep=": ")
  rename_sel <- snpeff_subset$ANN %in% c("upstream_gene_variant","downstream_gene_variant")
  labels[SNP %in% snpeff_subset$POS][rename_sel] <- paste0(snpeff_subset$REF, snpeff_subset$POS, snpeff_subset$ALT)[rename_sel]
  rename_sel <- snpeff_subset$ANN %in% c("synonymous_variant")
  ...
  colors[SNP %in% snpeff_subset$POS] <- annotation_colors$SnpEff[as.character(snpeff_subset$ANN)]

  sample.gr <- GRanges("MN908947.3", IRanges(SNP, width=1, names=labels))
  ...
  lolliplot(sample.gr, features, type="circle", xlab="genome position (nucleotide)", ylab="# of samples", xaxis=c(1,seq(5,25,5)*1000,29903), xaxis.gp = gpar(fontsize = 7), legend = list(annotation_colors$SnpEff))
...
#                      fill = rep(c('#2ca25f', '#99d8c9'), 10)[seq(1, length(genome_ref$genes))],
  ...
  for(i in 1:dim(mutation_subset)[2]) {
    ...
    labels <- mutation_subset[which(nchar(mutation_subset[,i])>0),i]
    labels[SNP %in% snpeff_subset$POS] <- paste(as.character(snpeff_subset$GENE), as.character(snpeff_subset$AA_MUT), sep=": ")
    rename_sel <- snpeff_subset$ANN %in% c("upstream_gene_variant","downstream_gene_variant")
    labels[SNP %in% snpeff_subset$POS][rename_sel] <- paste0(snpeff_subset$REF, snpeff_subset$POS, snpeff_subset$ALT)[rename_sel]
    rename_sel <- snpeff_subset$ANN %in% c("synonymous_variant")
    ...
    lolliplot(sample.gr, features, type="circle", xlab="genome position (nucleotide)", ylab="", xaxis=c(1,seq(5,25,5)*1000,29903), xaxis.gp = gpar(fontsize = 7))
  }
  ...
  if(length(flags) > 0) {
    for(flagString in flags) {
      ...
      if(nt != "see full output") {
        pngFn <- paste0(run_path,"/artic-pipeline/run_stats/igv/",sample_barcode,"/",sample_barcode,"-MN908947.3_",nt,".png")
...
## Genome cartoons
```{r, genome-cartoon-overview, eval=TRUE, echo=FALSE, fig.height=6, results="asis"}
genome_cartoon_overview(mutation_no_low_coverage)
```
...
#  cat('\n\n')
#}
genome_cartoons(mutation_no_low_coverage)
```
