...
if(length(list.files(run_path, "throughput*", full.names=TRUE)) > 0) {
	...
	throughput <- data.frame('Experiment.Time..minutes.' = seq(1,2), 'Reads' = rep(2,2), 'Basecalled.Reads.Passed' = rep(1,2), 'Basecalled.Reads.Failed' = rep(1,2))
}
...
if(file.exists(snpeff_reportFn)) {
  ...
  empty_column <- rep("unknown", dim(mutation_full)[1])
...
if(sum(select_many)>0) {
  ...
  if(dim(new_cols)[2] > 0) {
    dimnames(new_cols)[[2]] <- list(sapply(dimnames(mutation_table_many)[[2]], paste0, "*"))
...
if(length(levels(snpeff_report$ANN)) >= 3) {
	snpeff_colors <- c(brewer.pal(n=length(levels(snpeff_report$ANN)), name="Dark2"), "white")
} else {
	snpeff_colors <- rep("gray50", length(levels(snpeff_report$ANN)))
}
annotation_colors <- list(Flag = flagColors[flagColors != flagColors["NA"]], common.SNP = c("common"="#000000", "other"="#FFFFFF"), Variants = c("#FFFFFF", "gray30", "gray70"), SnpEff = snpeff_colors)

names(annotation_colors$SnpEff) <- "none" # list(levels(annotation_snp$SnpEff))
#-------------------------------------------------
```
...
depth_heatmap <- function(plot_sel, title_additional, norm=F) {
  ...
  sample_sel <- summary$Sample %in% gsub("(_NB[0-9]{2})?(\\*)?", "", rownames(plot_dat))
  ...
  pheatmap(as.data.frame(plot_dat),
           ...
           labels_col="",
           annotation_row=annotation_sample[sample_sel],
