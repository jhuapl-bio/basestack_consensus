...
if(length(list.files(run_path, "throughput*", full.names=TRUE) > 0) {
	...
	throughput <- data.frame('Experiment Time (minutes)' = 0, 'Reads' = 0, 'Basecalled Reads Passed' = 0, 'Basecalled Reads Failed' = 0)
}
...
depth_normFn <- paste0(run_path,"/artic-pipeline/run_stats/depth-norm-all.txt")
mutation_fullFn <- paste0(run_path, "/artic-pipeline/run_stats/mutations-table.txt")
...
if(file.exists(depth_allFn)) {
  ...
  for(i in levels(summary$Sample)) {
    depth_one <- data.frame(V1=rep(i, 29903), V2=rep("MN908947.3", 29903), V3=seq(1, 29903), V4=rep(1, 29903))
  }
  depth_all <- rbind(depth_all, depth_one)
}
if(file.exists(depth_normFn)) {
  depth_norm <- read.table(depth_normFn, sep="\t", fill=TRUE)
} else {
  depth_norm <- NULL
  for(i in levels(summary$Sample)) {
    depth_one <- data.frame(V1=rep(i, 29903), V2=rep("MN908947.3", 29903), V3=seq(1, 29903), V4=rep(1, 29903))
...
if(file.exists(snpeff_reportFn)) {
  ...
  snpeff_report <- data.frame("X.CHROM" = rep("MN908947.3", dim(mutation_full)[1]), "POS" = rownames(mutation_full), "REF" = empty_column, "ALT" = empty_column, "GENE" = empty_column, "ANN" = empty_column, "AA_MUT" = empty_column)
}
if(file.exists(clade_reportFn)) {
...
summary$Lineage <- rep("", dim(summary)[1])
if(dim(clade_report)[1]>0) {
  summary$Lineage[clade_report$taxon %in% summary$Sample] <- as.matrix(clade_report$lineage)
}
...
#  text_matrix[row_sel, col_sel] <- flag_matrix[flag_row_order[row_order[row_sel]], flag_col_order[col_order[col_sel]]]
  ...
  x <- rep(seq(0.095, 0.6, length.out=dim(text_matrix)[2]), each=dim(text_matrix)[1])
  y <- rep(seq(0.961, 0.129, length.out=dim(text_matrix)[1]), dim(text_matrix)[2])
  grid.text(text_matrix, x=x, y=y, gp=gpar(col="black", fontsize=5))
...
#  grid.text(paste(shapes$circles[3:7], collapse=""), x=0.58, y=0.131, gp=gpar(col="black", fontsize=7))
  grid.text("Variant position (nucleotide)", x=0.75, y=0.35, just="center", rot=-90, gp=gpar(col="black", fontsize=12))
}
#-------------------------------------------------
  ...
  if(length(flags) > 0) {
    for(flagString in flags) {
      ...
      if(nt != "see full output") {
        pngFn <- paste0(run_path,"/artic-pipeline/run_stats/igv/",sample,"/MN908947.3_",nt,".png")
