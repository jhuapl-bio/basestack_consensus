...
# ./prepare_for_GISAID_submission.py
...
${SCRIPTS_PATH}/prepare_for_GISAID_submission.py -in ${LINK} --api_key ${GOOGLE_API_KEY} --submission_manifest ${SUB_MANIFEST} --seq-path ${SEQ_PATH} --fasta-path ${FASTA_PATH} --submission-path ${SUBMISSIONS_PATH} --submitted-fasta ${SUBMITTED_SEQ} --master-meta ${MASTER_META} --resubmission
echo "DONE"
